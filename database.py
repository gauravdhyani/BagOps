...
import csv, hashlib, html, json, re, sqlite3, uuid
...
def connect():
    c=sqlite3.connect(SETTINGS.db_path,timeout=30);c.row_factory=sqlite3.Row
    c.execute('PRAGMA foreign_keys=ON');c.execute('PRAGMA journal_mode=WAL');c.execute('PRAGMA busy_timeout=5000');return c
@contextmanager
def tx():
    c=connect()
...
CREATE INDEX IF NOT EXISTS ix_queue ON claims(automation_status,lease_expires_at,immediate_attention,review_priority,submitted_at);
CREATE INDEX IF NOT EXISTS ix_nodes ON node_runs(graph_run_id,completed_at);
'''
MIGRATIONS = {
    'claims': {
        'passenger_status': 'TEXT', 'claim_status': "TEXT NOT NULL DEFAULT 'new'", 'submitted_at': 'TEXT', 'assigned_team': 'TEXT', 'review_reason': 'TEXT', 'created_at': 'TEXT', 'updated_at': 'TEXT', 'processing_token': 'TEXT', 'processing_started_at': 'TEXT',
        'lease_expires_at': 'TEXT', 'attempt_count': 'INTEGER NOT NULL DEFAULT 0',
        'last_worker_id': 'TEXT', 'review_priority': "TEXT NOT NULL DEFAULT 'STANDARD'",
        'immediate_attention': 'INTEGER NOT NULL DEFAULT 0', 'attention_reason': 'TEXT',
        'review_due_at': 'TEXT', 'ack_status': "TEXT NOT NULL DEFAULT 'NOT_SENT'",
    },
    'graph_runs': {
        'step_count': 'INTEGER NOT NULL DEFAULT 0', 'llm_calls': 'INTEGER NOT NULL DEFAULT 0',
        ...
        'provider_requests': 'INTEGER NOT NULL DEFAULT 0', 'provider_retries': 'INTEGER NOT NULL DEFAULT 0',
        'schema_repairs': 'INTEGER NOT NULL DEFAULT 0', 'cache_hits': 'INTEGER NOT NULL DEFAULT 0',
        'fallbacks': "TEXT NOT NULL DEFAULT '[]'", 'decision': 'TEXT', 'policy_result': 'TEXT',
        'error_code': 'TEXT', 'completed_at': 'TEXT', 'policy_config_id': 'TEXT', 'policy_snapshot': 'TEXT',
    },
    'node_runs': {
        'node_kind': "TEXT NOT NULL DEFAULT 'UNKNOWN'", 'invocation_reason': 'TEXT',
        'facts': "TEXT NOT NULL DEFAULT '{}'", 'evidence': "TEXT NOT NULL DEFAULT '[]'",
        'error_code': 'TEXT', 'scheduled_at': 'TEXT', 'deadline_at': 'TEXT', 'completed_at': 'TEXT',
        'duration_ms': 'INTEGER NOT NULL DEFAULT 0', 'input_tokens': 'INTEGER NOT NULL DEFAULT 0',
        'output_tokens': 'INTEGER NOT NULL DEFAULT 0', 'attempts': 'INTEGER NOT NULL DEFAULT 1',
        'cache_hit': 'INTEGER NOT NULL DEFAULT 0', 'provider_requests': 'INTEGER NOT NULL DEFAULT 0',
        'provider_retries': 'INTEGER NOT NULL DEFAULT 0', 'schema_repairs': 'INTEGER NOT NULL DEFAULT 0',
        'rejection_reason': 'TEXT',
    },
    'actions': {
        'actor_type': "TEXT NOT NULL DEFAULT 'SYSTEM'", 'payload_hash': "TEXT NOT NULL DEFAULT ''",
        'policy_authorization_id': 'TEXT', 'status': "TEXT NOT NULL DEFAULT 'UNKNOWN'",
        'request_payload': "TEXT NOT NULL DEFAULT '{}'", 'response_payload': "TEXT NOT NULL DEFAULT '{}'",
        'created_at': 'TEXT',
    },
    'classifications': {
        'phase': "TEXT NOT NULL DEFAULT 'LEGACY'", 'confidence': 'REAL', 'urgency': 'TEXT',
        'agreement': 'REAL', 'reason_codes': "TEXT NOT NULL DEFAULT '[]'", 'rationale': 'TEXT',
        'model_id': 'TEXT', 'prompt_version': 'TEXT', 'created_at': 'TEXT',
    },
    'policy_authorizations': {
        'reason_codes': "TEXT NOT NULL DEFAULT '[]'", 'evidence_hash': "TEXT NOT NULL DEFAULT ''",
        'approved_payload_hash': 'TEXT', 'policy_config_id': 'TEXT', 'authorization_mode': "TEXT NOT NULL DEFAULT 'LIVE'", 'consumed_at': 'TEXT', 'consumed_action_id': 'TEXT', 'created_at': 'TEXT',
    },
    'compensation_records': {
        'approved_amount': 'TEXT', 'paid_amount': 'TEXT', 'verified_discrepancy': 'TEXT',
        'currency': 'TEXT', 'record_status': 'TEXT', 'hold_flags': "TEXT NOT NULL DEFAULT '[]'",
        'prior_resolution_id': 'TEXT', 'raw_input': "TEXT NOT NULL DEFAULT '{}'",
        'validation_errors': "TEXT NOT NULL DEFAULT '[]'",
    },
    'communications': {
        'kind': 'TEXT', 'channel': "TEXT NOT NULL DEFAULT 'SIMULATED'", 'message': "TEXT NOT NULL DEFAULT ''",
        'delivery_status': "TEXT NOT NULL DEFAULT 'UNKNOWN'", 'created_at': 'TEXT',
    },
    'courtesy_resolutions': {
        'amount': 'TEXT', 'currency': 'TEXT', 'method': 'TEXT', 'status': 'TEXT', 'reason': 'TEXT', 'issued_at': 'TEXT',
    },
    'human_overrides': {
        'reviewer': 'TEXT', 'original_decision': "TEXT NOT NULL DEFAULT '{}'",
        'override_decision': "TEXT NOT NULL DEFAULT '{}'", 'reason': 'TEXT', 'created_at': 'TEXT',
    },
}
...
def init_db():
    with tx() as c:
        base_schema='\n'.join(line for line in SCHEMA.splitlines() if not line.strip().upper().startswith('CREATE INDEX'))
        c.executescript(base_schema)
        _apply_additive_migrations(c)
        c.execute("UPDATE claims SET created_at=COALESCE(created_at,CURRENT_TIMESTAMP)")
        c.execute("UPDATE claims SET updated_at=COALESCE(updated_at,created_at,CURRENT_TIMESTAMP)")
        c.execute('CREATE INDEX IF NOT EXISTS ix_queue ON claims(automation_status,lease_expires_at,immediate_attention,review_priority,submitted_at)')
        c.execute('CREATE INDEX IF NOT EXISTS ix_nodes ON node_runs(graph_run_id,completed_at)')
def digest(v:Any)->str:return hashlib.sha256(json.dumps(v,sort_keys=True,default=str).encode()).hexdigest()
def clean(v:Any)->str:return re.sub(r'<[^>]+>','',html.unescape(str(v or ''))).strip()[:12000]
def money(v):
    if not str(v or '').strip():return None,True
    try:x=Decimal(str(v).replace(',','.'));return (format(x,'f'),True) if x.is_finite() else (None,False)
    except InvalidOperation:return None,False
def ingest_claims(path):
    init_db();count=0
    with open(path,encoding='utf-8-sig',newline='') as f,tx() as c:
        for row in csv.DictReader(f):
            raw=dict(row);errors=[];cid=str(row.get('claim_id','')).strip()
            if not re.fullmatch(r'BC-\d{4,}',cid):errors.append('INVALID_CLAIM_ID');cid=cid or f'REJECTED-{digest(raw)[:12]}'
            ...
            ih=digest(raw);old=c.execute('SELECT input_hash,version FROM claims WHERE claim_id=?',(cid,)).fetchone();ver=1 if not old else old['version']+int(old['input_hash']!=ih);valid=not errors
            c.execute('''INSERT INTO claims(claim_id,passenger_name,passenger_status,claim_status,submitted_at,body_text,raw_input,input_hash,ingestion_status,validation_errors,version,automation_status,review_reason,review_priority) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(claim_id) DO UPDATE SET passenger_name=excluded.passenger_name,passenger_status=excluded.passenger_status,claim_status=excluded.claim_status,submitted_at=excluded.submitted_at,body_text=excluded.body_text,raw_input=excluded.raw_input,input_hash=excluded.input_hash,ingestion_status=excluded.ingestion_status,validation_errors=excluded.validation_errors,version=excluded.version,automation_status=CASE WHEN claims.input_hash<>excluded.input_hash THEN excluded.automation_status ELSE claims.automation_status END,updated_at=CURRENT_TIMESTAMP''',(cid,name or 'UNKNOWN',str(row.get('passenger_status') or row.get('passenger_tier') or 'Standard'),str(row.get('claim_status') or row.get('status') or 'new').lower(),submitted or None,body,json.dumps(raw),ih,'VALID' if valid else 'NEEDS_REVIEW',json.dumps(errors),ver,'PENDING' if valid else 'HUMAN_REVIEW',','.join(errors) or None,'STANDARD' if valid else 'HIGH'));count+=1
    return count
def ingest_compensation(path):
    init_db();count=0
    with open(path,encoding='utf-8-sig',newline='') as f,tx() as c:
        for row in csv.DictReader(f):
            raw=dict(row);errors=[];vals={}
            for field in ('approved_amount','paid_amount','verified_discrepancy'):
                vals[field],ok=money(row.get(field));errors += [] if ok else [f'INVALID_{field.upper()}']
            currency=str(row.get('currency','')).strip().upper()
            if currency and not re.fullmatch(r'[A-Z]{3}',currency):errors.append('INVALID_CURRENCY');currency=''
            rid=str(row.get('record_id') or row.get('source_record_id') or f'COMP-{digest(raw)[:12]}');holds=[x.strip().upper() for x in str(row.get('hold_flags','')).strip('[]').split(',') if x.strip()]
            c.execute('INSERT OR REPLACE INTO compensation_records(record_id,passenger_name,claim_id,approved_amount,paid_amount,verified_discrepancy,currency,record_status,hold_flags,prior_resolution_id,raw_input,validation_errors) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(rid,str(row.get('passenger_name','')).strip() or 'UNKNOWN',str(row.get('claim_id','')).strip() or None,vals['approved_amount'],vals['paid_amount'],vals['verified_discrepancy'],currency or None,str(row.get('record_status','')).lower() or None,json.dumps(holds),str(row.get('prior_resolution_id','')).strip() or None,json.dumps(raw),json.dumps(errors)));count+=1
    return count
def acquire(ids=None,limit=100,worker='local'):
    init_db();now=datetime.now(timezone.utc);lease=(now+timedelta(seconds=SETTINGS.lease_seconds)).isoformat();token=str(uuid.uuid4())
    with tx() as c:
        params=[];where="(automation_status='PENDING' OR (automation_status='PROCESSING' AND lease_expires_at<?))";params.append(now.isoformat())
        if ids:where+=f" AND claim_id IN ({','.join('?' for _ in ids)})";params.extend(ids)
        rows=c.execute(f'SELECT * FROM claims WHERE {where} ORDER BY submitted_at,claim_id LIMIT ?',(*params,limit)).fetchall();out=[]
        for row in rows:
            changed=c.execute("UPDATE claims SET automation_status='PROCESSING',processing_token=?,processing_started_at=?,lease_expires_at=?,attempt_count=attempt_count+1,last_worker_id=?,updated_at=CURRENT_TIMESTAMP WHERE claim_id=? AND (automation_status='PENDING' OR (automation_status='PROCESSING' AND lease_expires_at<?))",(token,now.isoformat(),lease,worker,row['claim_id'],now.isoformat())).rowcount
            if changed: item=dict(row);item['processing_token']=token;out.append(item)
        return out
...
def set_status(cid,status,reason=None,immediate=False,priority='STANDARD',processing_token=None):
    ...
    with tx() as c:
        if processing_token:
            result=c.execute('UPDATE claims SET automation_status=?,review_reason=?,immediate_attention=?,review_priority=?,attention_reason=?,review_due_at=?,processing_token=NULL,lease_expires_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE claim_id=? AND processing_token=? AND lease_expires_at>=?',(status,reason,int(immediate),priority,reason,due,cid,processing_token,datetime.now(timezone.utc).isoformat()))
            if result.rowcount != 1:
                raise LeaseOwnershipLost('PROCESSING_LEASE_OWNERSHIP_LOST')
        else:
            c.execute('UPDATE claims SET automation_status=?,review_reason=?,immediate_attention=?,review_priority=?,attention_reason=?,review_due_at=?,processing_token=NULL,lease_expires_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE claim_id=?',(status,reason,int(immediate),priority,reason,due,cid))
    return True
