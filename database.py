...
def connect():
 c=sqlite3.connect(SETTINGS.db_path,timeout=30);c.row_factory=sqlite3.Row;c.execute("PRAGMA foreign_keys=ON");c.execute("PRAGMA journal_mode=WAL");c.execute("PRAGMA busy_timeout=5000");return c
@contextmanager
def tx():
 c=connect()
...
"""
MIGRATIONS={"claims":{"passenger_status":"TEXT","claim_status":"TEXT NOT NULL DEFAULT 'new'","submitted_at":"TEXT","assigned_team":"TEXT","review_reason":"TEXT","created_at":"TEXT","updated_at":"TEXT","processing_token":"TEXT","processing_started_at":"TEXT","lease_expires_at":"TEXT","attempt_count":"INTEGER NOT NULL DEFAULT 0","last_worker_id":"TEXT","review_priority":"TEXT NOT NULL DEFAULT 'STANDARD'","immediate_attention":"INTEGER NOT NULL DEFAULT 0","attention_reason":"TEXT","review_due_at":"TEXT","ack_status":"TEXT NOT NULL DEFAULT 'NOT_SENT'"},"graph_runs":{"step_count":"INTEGER NOT NULL DEFAULT 0","llm_calls":"INTEGER NOT NULL DEFAULT 0","tool_calls":"INTEGER NOT NULL DEFAULT 0","input_tokens":"INTEGER NOT NULL DEFAULT 0","output_tokens":"INTEGER NOT NULL DEFAULT 0","duration_ms":"INTEGER NOT NULL DEFAULT 0","provider_requests":"INTEGER NOT NULL DEFAULT 0","provider_retries":"INTEGER NOT NULL DEFAULT 0","schema_repairs":"INTEGER NOT NULL DEFAULT 0","cache_hits":"INTEGER NOT NULL DEFAULT 0","fallbacks":"TEXT NOT NULL DEFAULT '[]'","decision":"TEXT","policy_result":"TEXT","error_code":"TEXT","completed_at":"TEXT","policy_config_id":"TEXT","policy_snapshot":"TEXT"},"node_runs":{"node_kind":"TEXT NOT NULL DEFAULT 'UNKNOWN'","invocation_reason":"TEXT","facts":"TEXT NOT NULL DEFAULT '{}'","evidence":"TEXT NOT NULL DEFAULT '[]'","error_code":"TEXT","scheduled_at":"TEXT","deadline_at":"TEXT","completed_at":"TEXT","duration_ms":"INTEGER NOT NULL DEFAULT 0","input_tokens":"INTEGER NOT NULL DEFAULT 0","output_tokens":"INTEGER NOT NULL DEFAULT 0","attempts":"INTEGER NOT NULL DEFAULT 1","cache_hit":"INTEGER NOT NULL DEFAULT 0","provider_requests":"INTEGER NOT NULL DEFAULT 0","provider_retries":"INTEGER NOT NULL DEFAULT 0","schema_repairs":"INTEGER NOT NULL DEFAULT 0","rejection_reason":"TEXT"}}
def _apply_additive_migrations(c):
...
def init_db():
 with tx() as c:
  c.executescript(SCHEMA);_apply_additive_migrations(c);c.execute("UPDATE claims SET created_at=COALESCE(created_at,CURRENT_TIMESTAMP)");c.execute("UPDATE claims SET updated_at=COALESCE(updated_at,created_at,CURRENT_TIMESTAMP)")
  c.execute("CREATE INDEX IF NOT EXISTS ix_queue ON claims(automation_status,claim_status,lease_expires_at,immediate_attention,review_priority,submitted_at)");c.execute("CREATE INDEX IF NOT EXISTS ix_nodes ON node_runs(graph_run_id,completed_at)")
...
def ingest_claims(path):
 ...
 with open(path,encoding="utf-8-sig",newline="") as f,tx() as c:
  for row in csv.DictReader(f):
   ...
   name=str(row.get("passenger_name","")).strip();body=clean(row.get("body_text"));submitted=str(row.get("submitted_at","")).strip()
   ...
   ih=digest(raw);old=c.execute("SELECT input_hash,version FROM claims WHERE claim_id=?",(cid,)).fetchone();version=1 if not old else old["version"]+int(old["input_hash"]!=ih);valid=not errors;claim_status=str(row.get("claim_status") or row.get("status") or "new").strip().lower();automation_status="PENDING" if valid else "HUMAN_REVIEW";reason=",".join(errors) or None
   c.execute("""INSERT INTO claims(claim_id,passenger_name,passenger_status,claim_status,submitted_at,body_text,raw_input,input_hash,ingestion_status,validation_errors,version,automation_status,review_reason,review_priority) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(claim_id) DO UPDATE SET passenger_name=excluded.passenger_name,passenger_status=excluded.passenger_status,claim_status=excluded.claim_status,submitted_at=excluded.submitted_at,body_text=excluded.body_text,raw_input=excluded.raw_input,input_hash=excluded.input_hash,ingestion_status=excluded.ingestion_status,validation_errors=excluded.validation_errors,version=excluded.version,automation_status=CASE WHEN claims.input_hash<>excluded.input_hash THEN excluded.automation_status ELSE claims.automation_status END,review_reason=CASE WHEN claims.input_hash<>excluded.input_hash THEN excluded.review_reason ELSE claims.review_reason END,updated_at=CURRENT_TIMESTAMP""",(cid,name or "UNKNOWN",str(row.get("passenger_status") or row.get("passenger_tier") or "Standard"),claim_status,submitted or None,body,json.dumps(raw),ih,"VALID" if valid else "NEEDS_REVIEW",json.dumps(errors),version,automation_status,reason,"STANDARD" if valid else "HIGH"));count+=1
 return count
def ingest_compensation(path):
 ...
 with open(path,encoding="utf-8-sig",newline="") as f,tx() as c:
  for row in csv.DictReader(f):
   ...
   for field in ("approved_amount","paid_amount","verified_discrepancy"):values[field],ok=money(row.get(field));errors+=[] if ok else [f"INVALID_{field.upper()}"]
   ...
   c.execute("INSERT OR REPLACE INTO compensation_records(record_id,passenger_name,claim_id,approved_amount,paid_amount,verified_discrepancy,currency,record_status,hold_flags,prior_resolution_id,raw_input,validation_errors) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",(rid,str(row.get("passenger_name","")).strip() or "UNKNOWN",str(row.get("claim_id","")).strip() or None,values["approved_amount"],values["paid_amount"],values["verified_discrepancy"],currency or None,str(row.get("record_status","")).lower() or None,json.dumps(holds),str(row.get("prior_resolution_id","")).strip() or None,json.dumps(raw),json.dumps(errors)));count+=1
 return count
def acquire(ids=None,limit=100,worker="local"):
 ...
 with tx() as c:
  params=[now.isoformat()];where="(automation_status='PENDING' OR (automation_status='PROCESSING' AND lease_expires_at<?)) AND LOWER(TRIM(COALESCE(claim_status,'')))!='closed'"
  ...
  for row in rows:
   changed=c.execute("""UPDATE claims SET automation_status='PROCESSING',processing_token=?,processing_started_at=?,lease_expires_at=?,attempt_count=attempt_count+1,last_worker_id=?,updated_at=CURRENT_TIMESTAMP WHERE claim_id=? AND LOWER(TRIM(COALESCE(claim_status,'')))!='closed' AND (automation_status='PENDING' OR (automation_status='PROCESSING' AND lease_expires_at<?))""",(token,now.isoformat(),lease,worker,row["claim_id"],now.isoformat())).rowcount
   if changed:item=dict(row);item["processing_token"]=token;out.append(item)
  return out
class LeaseOwnershipLost(RuntimeError):pass
def set_status(cid,status,reason=None,immediate=False,priority="STANDARD",processing_token=None):
 ...
 with tx() as c:
  ...
  else:c.execute("UPDATE claims SET automation_status=?,review_reason=?,immediate_attention=?,review_priority=?,attention_reason=?,review_due_at=?,processing_token=NULL,lease_expires_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE claim_id=?",(*args,cid))
 return True
