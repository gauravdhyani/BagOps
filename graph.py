...
class Graph:
    ...
    def consume(self,r):
        ...
        if not ok:
            if r["kind"]=="ACTION" and r["status"]=="SUCCESS" and reason=="NODE_DEADLINE_EXCEEDED": r.setdefault("facts",{})["warning_code"]="ACTION_COMPLETED_AFTER_DEADLINE"; r.setdefault("evidence",[]).append("ACTION_COMPLETED_AFTER_DEADLINE")
            else:
    ...
    def run_parallel(self,tasks):
        for task in tasks: self.reserve(task[1])
        with ThreadPoolExecutor(max_workers=len(tasks)) as pool: done=[future.result() for future in as_completed([pool.submit(self.task,*task) for task in tasks])]
        if self.arrival_order: done.sort(key=lambda result:self.arrival_order.index(result["node"]) if result["node"] in self.arrival_order else 999)
        for result in done: self.consume(result)
    def add(self,*task): self.reserve(task[1]); result=self.task(*task); return result,self.consume(result)
...
def run_agent(claim,trigger="AUTOMATION",dry_run=False,failures=None,delays=None,arrival_order=None):
    ...
    try:
        if str(claim.get("claim_status","")).strip().lower()=="closed":
            decision={"primary_category":None,"confidence":1.0,"agreement":1.0,"urgency":"L","destination_team":None,"recommended_action":"ACKNOWLEDGE","reason_codes":["CLAIM_ALREADY_CLOSED"],"rationale":"Claim already closed.","disposition":"CLAIM"}
            g.finalize_claim("PROCESSED","Claim already closed",False,"STANDARD")
            duration=int((time.perf_counter()-g.started)*1000)
            with tx() as c: c.execute("UPDATE graph_runs SET status='COMPLETED',step_count=0,llm_calls=0,tool_calls=0,input_tokens=0,output_tokens=0,duration_ms=?,fallbacks='[]',decision=?,policy_result='{}',completed_at=CURRENT_TIMESTAMP WHERE graph_run_id=?",(duration,json.dumps(decision),g.gid))
            return {"graph_run_id":g.gid,"status":"COMPLETED","decision":decision,"policy":{},"facts":{"claim_status":"closed"},"results":[],"rejected":[]}
        ...
        if needs: g.run_parallel([(tool_map[n],"TOOL",f"{n} requested by triage/readiness",TOOLS[tool_map[n]],claim["claim_id"],claim.get("passenger_name","")) if n=="COMP" else (tool_map[n],"TOOL",f"{n} requested by triage/readiness",TOOLS[tool_map[n]],claim["claim_id"]) for n in sorted(needs) if n in tool_map])
        ...
        with tx() as c:
            ...
            for related in facts.get("related_claims",[]):
                relation="EXACT_DUPLICATE" if related.get("claim_id")==facts.get("duplicate_candidate") else "RELATED"; c.execute("INSERT INTO claim_relationships(relationship_id,graph_run_id,claim_id,related_claim_id,relationship_type,evidence,similarity) VALUES(?,?,?,?,?,?,?)",(f"REL-{uuid.uuid4().hex}",g.gid,claim["claim_id"],related.get("claim_id"),relation,json.dumps(related.get("evidence",[])),related.get("similarity")))
